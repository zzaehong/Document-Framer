...
import { buildContextUnits, CONTEXT_BUILDER_VERSION as CHUNKER_VERSION } from '../src/context';
import { StructuralEngine } from '../src/structure';
...
const planContexts = async (text: string) => ({ chunks: await buildContextUnits(text, await new StructuralEngine().generate(source(text))) });
...
test('cross-chunk Korean names consolidate using existing candidate expressions', async () => {
  ...
  const framer = new GeminiFramer(new GeminiClient(async request => {
    ...
    if (input.contextMarkdown) return envelope(extraction([concept(chunk++ === 0 ? '분산투자' : '포트폴리오 분산')]));
  ...
  for (const [index, a] of journal.attempts.entries()) {
    ...
    if (a.trace?.stage === 'concept-extraction') {
      assert.equal(a.trace.contextUnitId, a.trace.chunkId);
      assert.match(a.trace.sectionId!, /^section-/);
    }
...
test('E: normalized duplicates use deterministic merge without a consolidation API call', async () => {
  ...
  const framer = new GeminiFramer(new GeminiClient(async request => {
    ...
    return input.contextMarkdown ? envelope(extraction([concept(calls === 1 ? 'Diversification' : 'DIVERSIFICATION')])) : envelope(classification);
  }));
  const preview = await framer.generate(source(twoSections), 'key');
...
test('section context preserves heading paths and source order', async () => {
  assert.equal((await planContexts('# A\n\nsmall\n\n## B\n\nsmall')).chunks.length, 1);
  const sections = (await planContexts(twoSections)).chunks;
  assert.equal(sections.length, 2);
  const large = (await planContexts(`# A\n\n${'text '.repeat(10000)}`)).chunks;
  assert.ok(large.length > 1);
  assert.ok(large.every(chunk => chunk.headingPath.includes('A')));
});
...
test('G: old 64 KiB / 128-block limit is replaced by multiple bounded requests', async () => {
  ...
  const framer = new GeminiFramer(new GeminiClient(async request => {
    ...
    if (input.contextMarkdown) {
      assert.ok(byteLength(input.contextMarkdown) <= BUDGET.maxChunkBytes);
...
test('I: partial extraction and consolidation failure abort the entire run without later calls', async () => {
  for (const failingStage of ['extraction', 'consolidation', 'document']) {
    ...
    const framer = new GeminiFramer(new GeminiClient(async request => {
      ...
      if (input.contextMarkdown) return envelope(extraction([concept(`Concept ${count}`)]));
...
test('retry records across multiple calls share document run but never overwrite earlier call attempts', async () => {
  ...
  const framer = new GeminiFramer(new GeminiClient(async request => {
    ...
    return envelope(input.contextMarkdown ? extraction([]) : classification);
  }, async () => {}, 1000, journal));
  const preview = await framer.generate(source(twoSections), 'key');
...
test('repository PRD, reading note and project-like long docs are processed offline within chunk budget', async () => {
  ...
  for (const text of samples) {
    const planned = await planContexts(text);
    assert.ok(planned.chunks.length <= BUDGET.maxChunks);
    const framer = new GeminiFramer(new GeminiClient(async request => envelope(inputOf(request).contextMarkdown ? extraction([]) : classification)));
    const preview = await framer.generate(source(text), 'key');
...
test('candidate output budgets fail explicitly without dropping concepts or calling consolidation', async () => {
  ...
  let calls = 0;
  const framer = new GeminiFramer(new GeminiClient(async request => {
    calls++; const input = inputOf(request); assert.ok(input.contextMarkdown);
...
test('maximum planned document stays within 34 logical requests and 102 HTTP attempts including retries', async () => {
  ...
  assert.equal((await planContexts(text)).chunks.length, BUDGET.maxChunks);
  let calls = 0;
  const journal = new AttemptJournal(async () => {});
  const framer = new GeminiFramer(new GeminiClient(async request => {
    calls++;
    ...
    const input = inputOf(request);
    if (input.contextMarkdown) return envelope(extraction([concept(`Concept ${calls}`)]));
    if (input.candidates) return envelope({ groups: input.candidates.map((c: any) => ({ concept: c.concept, candidateIds: [c.candidateId] })) });
    return envelope(classification);
  }, async () => {}, 1000, journal));
  const preview = await framer.generate(source(text), 'key');
