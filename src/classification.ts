/**
 * AI 분류 계약: Domain 경로·출처와 Content Nature의 응답 스키마 → 실제 값 검증.
 * Domain은 문서 분야, Content Nature는 정보·의견 중심의 글 성격을 뜻한다.
 */
import { Domain, decodeDomains, domainKey, isOther, validateDomainPath, MAX_DOMAIN_DEPTH, MAX_DOMAIN_NAME } from './domains';

// 고정 Domain 목록이 아니라 Domain 정책 및 Content Nature의 버전이다.
export const TAXONOMY_VERSION = 'domain-policy-v1/content-nature-v1';
export const RESPONSE_SCHEMA_VERSION = 'document-classification-schema-v4';
export interface DomainClassification extends Domain { source: 'existing' | 'new' | 'unclassified'; confidence: number }
export const CONTENT_NATURES = ['information', 'opinion', 'mixed', 'unclassified'] as const;
export type ContentNature = typeof CONTENT_NATURES[number];
export const CONTENT_NATURE_NAMES: Record<ContentNature, string> = {
  information: '정보', opinion: '의견', mixed: '정보 + 의견', unclassified: '분류 어려움',
};
// AI 응답에는 로컬 메타데이터 대신 분류값만 허용한다.
export interface Classification {
  domains: DomainClassification[];
  contentNature: { id: ContentNature; confidence: number };
}
...
export const RESPONSE_SCHEMA = object({
  ...
  contentNature: annotation(CONTENT_NATURES),
});
...
export function validateClassification(value: unknown, existingDomains: readonly Domain[] = []): Classification {
  const invalid = () => { throw new Error('분류 응답 검증 실패: Domain, Content Nature 또는 confidence 형식을 확인하세요.'); };
  ...
  const root = record(value, ['domains', 'contentNature']);
  ...
  const contentNature = checkedAnnotation(root.contentNature, CONTENT_NATURES);
  return { domains, contentNature } as Classification;
}
