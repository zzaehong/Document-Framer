...
4. 마지막 편집 후 60초가 지나면 분류합니다. **최근 Gemini 미리보기 열기**에서 Domain·Type·Unit·Label·confidence·원문 행 범위와 JSON을 확인합니다.
5. 원문과 기존 저장 Frame이 유지되는지 확인합니다. 미리보기는 요청 당시 원문 기준이며 메모리에만 남습니다. 재시작하면 사라집니다.
...
- 저장 위치: `<Vault 설정 폴더>/plugins/document-framer/data.json`. `version: 2`에 설정(모델·비밀 항목 ID), 기존 Frame, 독립적인 호출 시도 기록을 저장합니다. 키 값은 포함하지 않습니다. 기존 `version: 1`은 그대로 읽고 다음 저장 시 Frame을 보존하여 이전합니다. 원문을 수정하지 않으며 재시작 후 **Frame 보기** 명령으로 다시 조회 가능합니다.
